...
class ImpressaoChecklistRecebimento(Base):
    ...
    id_checklist = Column(
        Integer, ForeignKey("checklist_recebimento.id"), nullable=False
    )  # Chave estrangeira para 'checklist_recebimento'
    ...
    recebimento_id = Column(
        Integer, ForeignKey("recebimentos.id")
    )  # Chave estrangeira para Recebimento

    # Relacionamentos
    checklist = relationship(
        "ChecklistRecebimento", back_populates="impressao_checklists", lazy=True
    )
    ...
    usuario = relationship(
        "User",
        back_populates="impressao_checklists",

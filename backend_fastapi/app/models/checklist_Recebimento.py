...
class ChecklistRecebimento(
    settings.Base
):  # Substituindo Base de 'datapy' para o Base do app
    ...
    id_Recebimento: Mapped[int] = mapped_column(
        Integer, ForeignKey("recebimentos.id"), nullable=False
    )  # ID do recebimento
    ...
    notaInterna: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True
    )  # Nota interna (opcional)
    qUEIXA_CLIENTE: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True
    )  # Queixa do cliente (opcional)
    dataChecklist_OrdemServicos: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, default=lambda: datetime.now(SP_TZ)
    )  # Data do checklist
    usuario_id: Mapped[Optional[int]] = mapped_column(
        Integer, ForeignKey("usuario.id")
    )  # Chave estrangeira de usuários
    LINK_PDF_CHECKLIST: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True
    )  # Link do PDF do checklist
    Status_Checklist: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # Status do checklist (ex: 'Concluído', 'Em andamento', etc.)

    # Relacionamentos
    recebimento: Mapped["Recebimento"] = relationship(
        "Recebimento", back_populates="checklists"
    )
    ...
    impressao_checklists: Mapped[list] = relationship(
        "ImpressaoChecklistRecebimento", lazy=True
    )

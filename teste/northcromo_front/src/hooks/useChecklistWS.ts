...
export interface Checklist {
  ...
  // outros campos que estiverem no seu schema
}
...
const fetchChecklists = async (
  ...
  const res = await fetch(`${API_URL}?page=${page}&limit=${limit}&com_pdf=${com_pdf}`);
...
export default function useChecklistWS(
  ...
  const checklistsQuery = useQuery({
    ...
    queryFn: () => fetchChecklists(page, limit, com_pdf),
    staleTime: 1000 * 60 * 5,
  ...
  useEffect(() => {
    ...
    ws.onerror = (error) => console.error('❌ Erro WebSocket Checklist:', error);
    ...
    const debounceInvalidate = () => {
      if (timeout) return;
      timeout = setTimeout(() => {
        queryClient.invalidateQueries(['checklists', page, limit, com_pdf]);
        timeout = null;
      }, 1000);
    };

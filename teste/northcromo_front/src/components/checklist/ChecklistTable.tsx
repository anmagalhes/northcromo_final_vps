...
interface ChecklistTableProps {
  ...
  onEditarEmMassa: (checklists: Checklist[]) => void
}
    ...
    <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
            ...
            <div className="flex flex-wrap gap-3 items-center">
              ...
              {checklists.some(item => selecionados.includes(item.id) && !item.tem_pdf) && (
                <ButtonGerarPDFs
                  ...
                  //onGenerate={generatePDFsForSelectedOperations}
                />
              )}
      ...
      <table className="w-full table-auto border border-gray-200 rounded-md shadow-sm">
        ...
        <tbody>
          {loading ? (
            <tr>
              <td colSpan={5} className="text-center p-4">Carregando checklists...</td>
            </tr>
          ) : checklists.length === 0 ? (
            <tr>
              <td colSpan={5} className="text-center p-4">Nenhum checklist cadastrado.</td>
            </tr>
          ) : (
                ...
                >
                  ...
                  <td className="px-4 py-2">
                    {(estaEditando || emEdicaoMultipla) ? (
                      <input
                        type="text"
                        ref={el => (refsInputs.current[baseIndex + 0] = el)}
                        value={editavel?.recebimento_id || ''}
                        onChange={e =>
                          setEditaveis(prev =>
                            prev.map(c => c.id === item.id ? { ...c, recebimento_id: e.target.value } : c)
                          )
                        }
                  ...
                  <td className="px-4 py-2">
                    {(estaEditando || emEdicaoMultipla) ? (
                      <input
                        type="text"
                        ref={el => (refsInputs.current[baseIndex + 1] = el)}
                  ...
                  <td className="px-4 py-2 text-center">
                    {(estaEditando || emEdicaoMultipla) ? (
                      <input
                        type="checkbox"
                        ref={el => (refsInputs.current[baseIndex + 2] = el)}
      ...
      {modoEdicaoMultipla && (
        <div className="flex gap-4 mt-4">
          <button
            onClick={() => {
              const editados = editaveis.filter(c => selecionados.includes(c.id))

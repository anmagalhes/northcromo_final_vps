...
const gerarNomeFoto = (ordemIndex: number, nomeCliente: string, numeroOrdem: string): string => {
  const hoje = new Date();
  ...
  const handleFotoEscolher = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0 && fotos.length < 4) {
      ...
      reader.onloadend = () => {
        ...
        const novaFoto: Foto = {
          id: `${nomeCliente}_foto_${fotos.length + 1}`,
          uri: base64,
          base64,
        };
      ...
      <div className="fotos-container">
        ...
        {fotos.length < 4 && (
          <div className="opcoes-foto">
            ...
            />
            <button onClick={() => document.querySelector('.input-foto')?.click()} className="tirar-foto">
              Escolher Foto
            </button>
          </div>
        )}
        {fotos.length === 4 && <span className="limite-fotos">Você atingiu o limite de 4 fotos</span>}
      </div>
      {modalVisible && fotoTemp && (
        <div className="modal">
          <div className="overlay" onClick={closeModal}></div>
          <div className="modal-content">
            <img src={fotoTemp} alt="Foto ampliada" className="foto-ampliada" />

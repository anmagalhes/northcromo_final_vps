...
from app.utils import get_next_sequence  # Função para gerar sequência de fotos
...
async def upload_photo(
    ...
    try:
        ...
        # Verificar se a ordem existe no banco de dados
        order = db.query(Order).filter(Order.id == order_id).first()
        if not order:
            raise HTTPException(status_code=404, detail="Ordem não encontrada")

        # Fazer o upload da foto para o Google Drive
        file_content = await file.read()
        file_id = upload_file_to_drive(file.filename, file_content)

        # Gerar o link da foto
        file_link = generate_file_link(file_id)

        # Gerar a sequência da foto
        sequence = get_next_sequence(order_id, db)

        # Criar o objeto Foto e salvar no banco de dados
        new_photo = Photo(
            order_id=order_id,
